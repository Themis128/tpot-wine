...
# --- Absolute Paths ---
BASE_DIR = "/home/tbaltzakis/tpot-wine/AutoML_1/39_RandomForest"
CLEAN_DATA_PATH = f"{BASE_DIR}/data/cleaned_combined_data.csv"
MODEL_PATH = f"{BASE_DIR}/models/version_latest/model.pkl"
FEATURE_NAMES_PATH = f"{BASE_DIR}/models/version_latest/feature_names.txt"
METADATA_PATH = f"{BASE_DIR}/models/version_latest/model_timestamp.txt"
ASSETS_PATH = f"{BASE_DIR}/assets"
RETRAIN_SCRIPT = f"{BASE_DIR}/scripts/refit_model_with_readable_names.py"
...
def load_cleaned_data():
    return pd.read_csv(CLEAN_DATA_PATH, decimal=",")
...
def evaluate_model(model, X, y):
    ...
    rmse = root_mean_squared_error(y, preds)  # Use the new function
...
with tab4:
    ...
    if uploaded:
        try:
            user_df = pd.read_csv(uploaded, decimal=",")
...
with tab5:
    ...
    image_paths = {
        "Normalized Confusion Matrix": f"{ASSETS_PATH}/confusion_matrix_normalized.png",
        "Confusion Matrix": f"{ASSETS_PATH}/confusion_matrix.png",
        "ROC Curve": f"{ASSETS_PATH}/roc_curve.png",
        "Precision-Recall Curve": f"{ASSETS_PATH}/precision_recall_curve.png",
        "Learning Curve": f"{ASSETS_PATH}/learning_curves.png"
    }
